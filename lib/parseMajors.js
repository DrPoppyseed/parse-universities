...
async function parseMajorsToJson(fileName, schoolNameCamel, schoolOpeid) {
  ...
  return await csv().fromFile(fileName).then(csvData => {
    return csvData.filter(item => {
      if (item.OPEID6 == schoolOpeid) return true;else return false;
    }).map(item => {
      return {
        opeid6: item.OPEID6,
...
async function parseMajorsByUnitid(fileName, schoolNameCamel, unitid) {
  return await csv().fromFile(fileName).then(csvData => {
    return csvData.filter(item => {
      ...
      if (localUnitid == unitid) return true;else return false;
    }).map(item => {
      // console.log(item)
      let localUnitid;
...
async function createSchoolObj(fromFileNum, toFileNum, files, schoolNameCamel, schoolOpeid) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const result = await parseMajorsToJson(fileName, schoolNameCamel, schoolOpeid);
    accum = [...accum, ...result];
    if (fileNum === toFileNum) _nestMajors(schoolNameCamel, accum);else fileNum = files[i];
  }
}

async function createSchoolObjUnitid(fromFileNum, toFileNum, files, schoolNameCamel, unitid) {
  const FILE_NAME_BASE = "./csv/majorsAndMinors/majorsAndMinorsCsv";
  let accum = [];
  let fileNum;
  ...
  for (let i = 0; i < files.length; i++) {
    fileNum = files[i];
    const fileName = `${FILE_NAME_BASE}${fileNum}.csv`;
    const result = await parseMajorsByUnitid(fileName, schoolNameCamel, unitid);
    accum = [...accum, ...result]; // console.log(schoolNameCamel, accum);

    if (fileNum === toFileNum) _nestMajorsUnitid(schoolNameCamel, accum); // else fileNum = files[i];
  }
}
...
function _parsedMajor(major) {
  ...
  return {
    ...
    credDesc,
    graduates,
    medianIncome1: medianIncome1 == "PrivacySuppressed" ? "empty" : medianIncome1,
    medianIncome2: medianIncome2 == "PrivacySuppressed" ? "empty" : medianIncome2
  };
}
...
async function parseMajorsWithMap(filePath) {
  ...
  for (let i = 0; i < parsedData.length; i++) {
    ...
    await createSchoolObj(Math.min(...fileNums), Math.max(...fileNums), sortedFiles, opeid, opeid);
  }
}
...
async function parseMajorsWithMapUnitid(filePath) {
  ...
  for (const item of parsedData) {
    ...
    await createSchoolObjUnitid(Math.min(...fileNums), Math.max(...fileNums), sortedFiles, unitid, unitid);
  }
}
