...
function parseInstitutions(from, to) {
  for (let i = from; i <= to; i++) {
    ...
    csv().fromFile(filePath).then(async res => {
      ...
      fs.writeFile(filename, JSON.stringify(item), err => {
        if (err) console.log(err);else console.log(`${filename}`);
      });
    }).catch(err => {
      console.log(err);
    });
...
async function parseInstitutionsAndOutputNameTxt(from, to) {
  ...
  for (let i = from; i <= to; i++) {
    // console.log(i);
    const filePath = `./csv/institutionAllData/institution${i}.csv`;
    const tempArr = await csv().fromFile(filePath).then(async res => {
...
function _extractInfoFromCsv(obj) {
  ...
  const unique_flags = miscHelper.schoolTypeFlag([["HBCU・歴史的黒人大学", obj.HBCU], ["PBI・", obj.PBI], ["ANNHI・", obj.ANNHI], ["TRIBAL", obj.TRIBAL], ["AANAPII", obj.AANAPII], ["HSI", obj.HSI], ["NANTI", obj.NANTI], ["男子校", obj.MENONLY], ["女子校", obj.WOMENONLY]]);
  const other = miscHelper.calcStudentDemoOther([obj.UGDS_WHITE, obj.UGDS_BLACK, obj.UGDS_HISP, obj.UGDS_ASIAN, obj.UGDS_NRA]);
  return {
    unitid: obj.UNITID,
    opeid: obj.OPEID,
    opeid6: obj.OPEID6,
    ...
    institutionData: {
      ...
      sat: {
        ...
        average: obj.SAT_AVG !== "NULL" || obj.SAT_AVG === 0 ? ~~obj.SAT_AVG : "-",
        high: obj.SATVR75 !== "NULL" || obj.SATVR75 === 0 ? ~~obj.SATVR75 + ~~obj.SATMT75 : "-"
      },
      cost: {
        academic_year_avg: academic_year_avg === "NaN" ? "-" : academic_year_avg,
        in_state_tuition: in_state_tuition === "NaN" ? "-" : in_state_tuition,
        out_of_state_tuition: out_of_state_tuition === "NaN" ? "-" : out_of_state_tuition
      },
      students: {
        size: size === "NaN" ? "-" : size,
        men: !!men ? men : "-",
        ...
        demographics: {
          ...
          international: !!international || international === 0 ? international : "-",
...
module.exports.parseInstitutionsAndOutputNameTxt = parseInstitutionsAndOutputNameTxt;
