...
/** CONSTANTS */
const CRED_DESC_JAP_MAP = new Map([
  ["1", "学士認定"],
  ["2", "准学士"],
  ["3", "学士"],
  ["4", "ポストバカロレア認定"],
...
const prettifyParsedMajors = (filepath: string) => {
  ...
  if (parsedData.majors.length === 1) {
    obj.majors.push({
      ..._hasOnlyOneMajor(parsedData.majors[0], parsedCIPData, deptCode),
    });
  } else {
...
/** HELPER METHODS */
const _hasOnlyOneMajor = (major: Object, parsedCIPData, deptCode): Object => {
  const { cipCode, credLev } = major;
  return {
    ..._getMajorCIPData(parsedCIPData, cipCode, deptCode),
    programsPerCredLevInDept: { ...TRACKER_INTIAL, [`${~~credLev}`]: 1 },
    programs: [...DEPT_INITIAL.programs],
  };
};

const _getMajorCIPData = (
  parsedCIPData: Object,
  cipCode: string,
  deptCode: string
): Object => {
  const localDeptCode = _getLocalDeptCode(cipCode);

  const { major, description, majorTitleJap } = _parseAndLookupCIP(
    parsedCIPData,
    localDeptCode
  );
...
const _getLocalDeptCode = (code: string): string => {
  if (code.length >= 2) {
...
const _getLocalProgramCode = (code: string): string => {
  if (code.length >= 2) {
...
function _parseAndLookupCIP(data: Object, target: string): Object {
